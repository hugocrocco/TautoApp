    ...
    public void ensureTables() {
        jdbc.execute(
                "CREATE TABLE IF NOT EXISTS messages (" +
                        ...
                        "INDEX idx_messages_inst_rut " +
                        "(institucion_id, recipient_rut)" +
                        ")"
        );
    }

    @GetMapping
    public Map<String, Object> listForMember(
        ...
        List<Map<String, Object>> memberRows = jdbc.queryForList(
                "SELECT estado_sindicato " +
                        "FROM members " +
                        "WHERE institucion_id = ? AND rut = ? " +
                        "LIMIT 1",
                institutionId,
                normalizedRut
        );

        String memberStatus = memberRows.isEmpty()
                ? ""
                : stringValue(
                        memberRows.get(0).get("estado_sindicato")
                ).toUpperCase();
